...
# -------- Config --------
# NOTE: Secrets must NOT be hardcoded. Provide these via environment variables
# (e.g. using a local `.env` file that is gitignored, or system env vars).
ANGEL_API_KEY = os.getenv("ANGEL_API_KEY")
ANGEL_USER_ID = os.getenv("ANGEL_USER_ID")
ANGEL_TOTP_SECRET = os.getenv("ANGEL_TOTP_SECRET")

_angel_pin_raw = os.getenv("ANGEL_PIN")
ANGEL_PIN = int(_angel_pin_raw) if _angel_pin_raw else None

def _mask(s: str, keep_last: int = 2) -> str:
    """Mask sensitive strings for logs (keep last N chars)."""
    if not s:
        return ""
    if len(s) <= keep_last:
        return "*" * len(s)
...
def _require_env(name: str, value):
    ...
    if value is None or (isinstance(value, str) and not value.strip()):
        raise RuntimeError(
            ...
            f"Set it in your environment (recommended: a local .env file)."
        )


# Fail fast at startup if secrets are not configured
_require_env("ANGEL_API_KEY", ANGEL_API_KEY)
_require_env("ANGEL_USER_ID", ANGEL_USER_ID)
_require_env("ANGEL_PIN", ANGEL_PIN)
_require_env("ANGEL_TOTP_SECRET", ANGEL_TOTP_SECRET)

# Local paths (Windows compatible)
SCRIPT_DIR = Path(__file__).parent
LOCAL_DB_DIR = SCRIPT_DIR.parent / "data"
LOCAL_DB_DIR.mkdir(parents=True, exist_ok=True)
LOCAL_LOGS_DIR = SCRIPT_DIR.parent / "logs"
LOCAL_LOGS_DIR.mkdir(parents=True, exist_ok=True)

# Database URL - use local path (same as VPS, just different location)
DB_URL = os.getenv("DB_URL", f"sqlite:///{LOCAL_DB_DIR / 'nifty_local.db'}").strip()
...
def _is_pg():
    return DB_URL.lower().startswith("postgresql")
...
def init_db():
    ...
    for attempt in range(max_retries):
        try:
            ...
            # PostgreSQL: create indexes (UNIQUE already in DDL)
            if _is_pg():
                with engine.begin() as conn:
                    conn.exec_driver_sql("CREATE INDEX IF NOT EXISTS idx_ticks_symbol_ts ON ltp_ticks(symbol, ts);")
                    conn.exec_driver_sql("CREATE INDEX IF NOT EXISTS idx_ticks_ts ON ltp_ticks(ts);")
                    conn.exec_driver_sql("CREATE INDEX IF NOT EXISTS idx_ticks_symbol ON ltp_ticks(symbol);")
...
def angel_login():
    ...
    logger.info(f"✅ Logged in as {_mask(ANGEL_USER_ID)}")
...
def is_trading_day_from_csv(csv_path=None):
    ...
    try:
        ...
        if csv_path is None:
            ...
            # Use local data directory (Windows compatible)
            calendar_2025 = str(LOCAL_DB_DIR / "NSE_Market_Calendar_2025.csv")
            calendar_2024_25 = str(LOCAL_DB_DIR / "NSE_Market_Calendar_2024_25.csv")
        ...
        if csv_path and "2025.csv" in csv_path:
            ...
            return is_trading_day_from_csv(str(LOCAL_DB_DIR / "NSE_Market_Calendar_2024_25.csv"))
...
def insert_rows(table, rows):
    ...
    elif _is_pg() and table in ("ltp_ticks", "oi_snapshots"):
        update_cols = [c for c in cols if c not in ("id", "symbol", "ts")]
        set_clause = ", ".join([f"{c}=excluded.{c}" for c in update_cols]) or "symbol=excluded.symbol"
        sql = (
            f"INSERT INTO {table} ({', '.join(cols)}) VALUES ({placeholders}) "
...
def simplified_data_writer_loop(symbols):
    ...
    while RUN_FLAG.is_set():
        try:
            ...
            # Use canonical timestamp format (UTC, no timezone suffix) to avoid
            # mixed ts formats like `...+00:00` vs `...` causing:
            # - missed comparisons (ts > last_ts)
            # - duplicate rows (unique index is on (symbol, ts) string)
            now_iso = next_boundary.strftime("%Y-%m-%dT%H:%M:%S")
            batch = []
...
def main():
    ...
    # Use local logs directory (Windows compatible)
    LOCAL_LOGS_DIR.mkdir(parents=True, exist_ok=True)
    ...
    print(f"🔑 User ID: {_mask(ANGEL_USER_ID)}")
    print("=" * 60)
